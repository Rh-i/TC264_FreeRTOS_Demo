...
void device_servo_set_angle(DeviceServo *servo, int32 angle)
{
  ...
  // 计算占空比 - 线性映射角度到占空比
  // 公式: duty = 4823 + (angle×3079+42) / 84
  // -35° → 3540, 0° → 4823, +35° → 6106

  duty = 4823 + (angle * 3079 + 42) / 84;
...
void device_servo_all_init(void)
{
  ...
  // 机械中值: 4823（实测物理居中值）
  // 角度范围 ±35°
  // -35° → duty = 4823 + (-35×3079+42)/84 = 3540
  // +35° → duty = 4823 + (35×3079+42)/84 = 6106
